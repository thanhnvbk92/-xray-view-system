...
def get_dashboard_summary(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Lấy danh sách máy trước
    machines = db.query(Machine).all()
    
    # Lấy số lượng NG chưa confirm từ PCB
    ng_counts = db.query(
        PCB.machine_id,
        func.count(PCB.id).label('count')
    ).filter(
        PCB.final_result == "NG",
        PCB.user_confirmed == False
    ).group_by(PCB.machine_id).all()
    
    counts_map = {row.machine_id: row.count for row in ng_counts}
    
    summary = []
    for m in machines:
        count = counts_map.get(m.id, 0)
        summary.append({
            "id": m.id,
            "name": m.name,
...
@router.get("/stats")
def get_overall_stats(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for i in range(7):
        ...
        if d_str in current_trends:
            ...
            trends.append({
                ...
                "ok_rate": round((data["ok"] / total * 100), 1) if total > 0 else 0,
                "ai_ok_rate": round((data["ai_ok"] / total * 100), 1) if total > 0 else 0,
                "user_ok_rate": round((data["user_ok"] / total * 100), 1) if total > 0 else 0
            })
        else:
            trends.append({
                "date": d_str,
