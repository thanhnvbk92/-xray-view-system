    ...
    return (
      <div style={{ background: 'rgba(15, 23, 42, 0.95)', border: '1px solid var(--glass-border)', borderRadius: '12px', padding: '12px', color: '#fff', fontSize: '0.85rem', boxShadow: '0 10px 25px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
          ...
          <div style={{ color: '#22c55e' }}>OK: {okPercent}% ({data.ok || 0})</div>
          <div style={{ color: '#ef4444' }}>NG: {ngPercent}% ({data.ng || 0})</div>
          <div style={{ color: '#06b6d4', marginTop: '5px' }}>AI OK: {aiPercent}% ({ai_ok})</div>
          <div style={{ color: '#f59e0b' }}>User OK: {userPercent}% ({user_ok})</div>
  ...
  const fetchData = async () => {
    try {
      ...
      const trendsWithLabels = (resTrends.data || []).map(t => {
        const total = t.total || (t.ok + t.ng);
        const okPercNum = total > 0 ? (t.ok / total * 100) : 0;
        const userPercNum = total > 0 ? (t.user_ok / total * 100) : 0;
        
        return {
          ...t,
          ok_rate_val: okPercNum,
          user_ok_rate_val: userPercNum,
          okLabel: `${okPercNum.toFixed(1)}% (${t.ok})`,
          userLabel: `${userPercNum.toFixed(1)}% (${t.user_ok})`,
          rateLabel: `${t.ng_rate}%`
        };
      });
          ...
          <div style={{ height: '300px', width: '100%' }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trends} margin={{ top: 20, right: 50, left: 10, bottom: 10 }}>
                ...
                <YAxis yAxisId="left" stroke="#94a3b8" fontSize={11} tickLine={false} axisLine={false} domain={[0, 100]} tickFormatter={(v) => `${v}%`} />
                <Tooltip content={<CustomTooltip />} />
                <Area yAxisId="left" type="monotone" dataKey="ok_rate_val" stroke="#22c55e" fillOpacity={1} fill="url(#colorOk)" strokeWidth={3} name="Machine OK">
                  <LabelList dataKey="okLabel" position="top" style={{ fill: '#22c55e', fontSize: '10px', fontWeight: 'bold' }} offset={10} />
                </Area>
                <Line yAxisId="left" type="monotone" dataKey="user_ok_rate_val" stroke="#f59e0b" strokeWidth={2} name="User OK" dot={{ r: 4, fill: '#f59e0b' }}>
                  <LabelList dataKey="userLabel" position="top" style={{ fill: '#f59e0b', fontSize: '10px', fontWeight: 'bold' }} offset={10} />
                </Line>
              </AreaChart>
            </ResponsiveContainer>
          </div>
